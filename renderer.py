...
if 'PYOPENGL_PLATFORM' not in os.environ:
    os.environ['PYOPENGL_PLATFORM'] = 'opengl'
import torch
import numpy as np
import pyrender
import trimesh
...
class Renderer:
    ...
    def __call__(self,
        ...
        if full_frame:
            # 1. imgname이 문자열(경로)인 경우 파일에서 읽기
            if isinstance(imgname, str):
                loaded_img = cv2.imread(imgname)
                if loaded_img is not None:
                    image = loaded_img.astype(np.float32)[:, :, ::-1] / 255.
                else:
                    # 파일 로드 실패 시 예외 처리: image 인자로 넘어온 데이터 사용 시도
                    if isinstance(image, torch.Tensor):
                        image = image.cpu().numpy()
                    if image.dtype == np.uint8:
                        image = image.astype(np.float32) / 255.0
            # 2. imgname이 이미 Numpy 배열인 경우 (프레임 데이터가 직접 넘어온 경우)
            elif isinstance(imgname, np.ndarray):
                image = imgname.copy()
                if image.dtype == np.uint8:
                    image = image.astype(np.float32) / 255.0
            # 3. 그 외의 경우 (image 인자에 프레임이 이미 들어있는 경우)
            else:
                if isinstance(image, torch.Tensor):
                    image = image.cpu().numpy()
        ...
        # [FIX] Use PerspectiveCamera instead of IntrinsicsCamera for compatibility
        yfov = 2.0 * np.arctan(image.shape[0] / (2.0 * self.focal_length))
        camera = pyrender.PerspectiveCamera(yfov=yfov, aspectRatio=image.shape[1] / image.shape[0], zfar=1e12)
    ...
    def render_rgba(
        ...
        # camera_pose[:3, 3] = camera_translation
        # [FIX] Use PerspectiveCamera instead of IntrinsicsCamera for compatibility
        yfov = 2.0 * np.arctan(render_res[1] / (2.0 * self.focal_length))
        camera = pyrender.PerspectiveCamera(yfov=yfov, aspectRatio=render_res[0] / render_res[1], zfar=1e12)
    ...
    def render_rgba_multiple(
        ...
        # [FIX] Use PerspectiveCamera instead of IntrinsicsCamera for compatibility
        yfov = 2.0 * np.arctan(render_res[1] / (2.0 * focal_length))
        camera = pyrender.PerspectiveCamera(yfov=yfov, aspectRatio=render_res[0] / render_res[1], zfar=1e12)
